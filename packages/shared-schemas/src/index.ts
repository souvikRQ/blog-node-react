...
export const createCommentSchema = z
  .object({
    ...
    guestName: z.string().trim().min(2, 'Name must be at least 2 characters').max(50, 'Name cannot exceed 50 characters').optional(),
    guestEmail: z.string().trim().toLowerCase().email('Invalid email address format').optional(),
  })
  .refine(
    (data) => {
    ...
    {
      message: 'Both guest name and email are required for public comments',
      path: ['guestName'],
    }
  );
